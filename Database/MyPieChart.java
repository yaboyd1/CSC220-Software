...
public class MyPieChart {
	...
	// CONSTRUCTORS
	public MyPieChart(MyOval oval, Map<Character, Integer> events, int count, int sum) {
		freq = events;
		this.count = count;
		pie = new MyOval(oval);
		this.sum = sum;
	...
	public void draw(GraphicsContext GC) {
		...
		for (int i = 0; itr.hasNext() && i < count; ++i) {
			// Increment
			item = itr.next();
			shift = ROTATION * item.getValue() / (double)sum * 360;
		...
		if (itr.hasNext()) {
			...
			mid = Math.toRadians(mid);
			GC.fillText(String.format("%s, %.4f", "All other grades", 1 - total),
					pie.getX() + pie.getWidth() / 2 + r * Math.cos(mid), 
					pie.getY() + pie.getHeight() / 2 - r * Math.sin(mid));
		}
	}
